...
namespace Words {
  ...
  class RegularConstraintBuilder: public BaseVisitor {
    public:
      RegularConstraintBuilder(Words::Context& ctxt) : ctxt(ctxt) {}
      ...
      virtual void caseReIn ( ReIn& c){
        ...
        std::stringstream ss;
        root->toString(ss);
        std::cout << "\tParsed Regular Constraint: " << pattern << "⋵ " << ss.str() << "\n";
      ...
      virtual void caseReTo ( ReTo& c) {
        ...
        wordBuilder->flush();
        std::shared_ptr<Words::Regex::RegWord> rword(new Words::Regex::RegWord(w));
      ...
      virtual void caseReUnion ( ReUnion& c) {
        processRegOp(c, Words::Regex::RegularOperator::UNION);
      }

      virtual void caseReConcat ( ReConcat& c) {
        processRegOp(c, Words::Regex::RegularOperator::CONCAT);
      }

      virtual void caseReStar ( ReStar& c){
        processRegOp(c, Words::Regex::RegularOperator::STAR);
      }
    ...
    private:
      ...
      void processRegOp(DummyApplication& c, Words::Regex::RegularOperator op) {
        std::shared_ptr<Words::Regex::RegOperation> node(new Words::Regex::RegOperation(op));
      ...
      std::shared_ptr<Words::Regex::RegNode> root = nullptr; // Points to the root of the regex tree
      std::shared_ptr<Words::Regex::RegOperation> parent = nullptr; // Points to the current parent during construction
  ...
  private:
	...
	public:
      ...
      RegularConstraintBuilder rBuilder(*jg->context);
