...
namespace Words {
  namespace CVC4 {
    ...
    class CVC4IntegerSolver : public Words::SMT::IntegerSolver {
    public:
      CVC4IntegerSolver () : engine(&em),timeout(0) {	
	engine.setOption("produce-models", "true");
	engine.setOption("strings-exp", "true");
	// Set output language to SMTLIB2
	engine.setOption("output-language", "smt2");
	...
	if (l.isLinear ()) {
      ...
      virtual Words::SMT::SolverResult solve () {
	auto form  = em.mkConst<bool> (true);
	for (size_t i = 0; i < asserts.size(); i++) {
	  form = em.mkExpr (::CVC4::kind::AND,form,asserts[i]);
	}
	if (timeout) {
    ...
    private:
      ::CVC4::ExprManager em;
      ::CVC4::SmtEngine engine;
      std::unordered_map<const Words::IEntry*,::CVC4::Expr> exprs;
      std::vector<::CVC4::Expr> asserts;
      std::set<char> terminals;
      size_t timeout = 0;
    };
    ...
    Words::SMT::IntSolver_ptr makeCVC4IntSolver () {
      return std::make_unique<CVC4IntegerSolver> ();
    }
