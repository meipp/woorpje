  ...
  namespace Solvers {
	namespace SatEncoding {
	  ...
	  ::Words::Solvers::Result Solver::Solve (Words::Options& opt,::Words::Solvers::MessageRelay& relay)   {
		...
		return runSolver (false,opt.context,sub,timekeep,(diagnostic ? &diagStr : nullptr));
		
	  }
	  
	  //Should only be called if Result returned HasSolution
	  void Solver::getResults (Words::Solvers::ResultGatherer& r)  {
	...
	template<>
	Solver_ptr makeSolver<Types::SatEncoding> () {return std::make_unique<SatEncoding::Solver> ();}
  }
}
