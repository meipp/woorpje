...
class RegEmpty : public RegNode {
   public:
    ...
    std::shared_ptr<RegNode> derivative(std::string) override { return std::make_shared<RegEmpty>(RegEmpty()); }

    std::shared_ptr<RegNode> reverse() override { return std::make_shared<RegEmpty>(RegEmpty()); }
};
...
class RegWord : public RegNode {
   public:
    ...
    void toString(std::ostream &os) override {
        for (auto c : word) os << c->getTerminal()->getChar();
    }

    size_t characters() override { return word.characters(); }
...
class RegOperation : public RegNode {
   public:
    RegOperation(RegularOperator regOperator) : RegOperation(regOperator, std::vector<std::shared_ptr<RegNode>>{}){};

    RegOperation(RegularOperator regOperator, std::vector<std::shared_ptr<RegNode>> children) : op(regOperator), children(children){};

    /**
     * Adds a child (i.e. operand) to this node.
     */
    void addChild(std::shared_ptr<RegNode> node) { children.push_back(node); };
     ...
     */
    std::vector<std::shared_ptr<RegNode>> getChildren() { return children; };
