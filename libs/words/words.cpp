...
struct Context::Internals {
    ~Internals() {
        ...
        for (auto v : terminals) delete v;
        for (auto v : sequences) delete v;
    }
...
IEntry* Context::addVariable(const std::string& c) { return _internal->addVariable(c, this); }

IEntry* Context::addTemporaryVariable(const std::string& c) { return _internal->addTemporaryVariable(c, this); }

IEntry* Context::addTerminal(char c) { return _internal->addTerminal(c, this, false); }
...
bool Context::conformsToConventions() const {
    for (auto v : _internal->vars) {
        if (v->getName().size() != 1 || !std::isupper(v->getName()[0])) {
            return false;
        }
    }
    for (auto v : _internal->terminals) {
        if (v->isEpsilon()) continue;
        if (v->getName().size() != 1 || std::isupper(v->getName()[0])) {
            return false;
        }
    }
...
size_t Context::nbVars() const { return _internal->vars.size(); }

size_t Context::nbTerms() const { return _internal->terminals.size(); }

IEntry* Context::getTerminal(size_t s) const { return _internal->terminals[s]; }

IEntry* Context::getVariable(size_t s) const { return _internal->vars[s]; }
const std::vector<Terminal*>& Context::getTerminalAlphabet() const { return _internal->terminals; }
const std::vector<Variable*>& Context::getVariableAlphabet() const { return _internal->vars; }
IEntry* Context::findSymbol(const std::string& c) const {
    auto it = _internal->reprToEntry.find(c);
    if (it != _internal->reprToEntry.end()) return it->second;
...
Terminal* Context::getEpsilon() const { return _internal->terminals[0]; }

WordBuilder::~WordBuilder() { flush(); }

WordBuilder& WordBuilder::operator<<(char c) { return operator<<(std::string(1, c)); }
...
std::ostream& operator<<(std::ostream& os, const Substitution& sub) {
    os << "{ ";
    for (auto& s : sub) os << *s.first << " -> " << s.second << " ";
    return os << " }";
}
